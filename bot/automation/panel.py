...
# Панель называет запрещённое слово прямо в отказе, в кавычках:
# «Запрещено использовать «розыгрыш» в тексте. Пожалуйста, уберите это
# слово.» Кавычки бывают ёлочками, лапками и обычными — берём все.
_FORBIDDEN_WORD = re.compile(
    r"[Зз]апрещен[оы][^»\"'«]*?[«\"'“„]([^»\"'“”«]{2,40})[»\"'”“]")
...
def strip_words(text: str, words) -> str:
    """Убрать слова из текста ВМЕСТЕ С ОКОНЧАНИЯМИ, не склеив соседние.

    Окончания важны: панель ищет подстроку, а не словоформу. Убрав ровно
    «розыгрыш», мы оставили бы «розыгрыша» — и получили бы тот же отказ
    вторым заходом, то есть кнопка «убрать и создать» не работала бы, а
    выглядела бы работающей.

    Границу СЛЕВА считаем по самой кириллице: `\\b` перед ней после `\\n` в
    исходнике не работает — escape-последовательность это буква `n`, и
    граница слова там не считается (та же ловушка, что и в правилах голоса).
    """
    out = str(text or "")
    for word in words:
        if not word:
            continue
        out = re.sub(
            r"(?<![А-Яа-яЁёA-Za-z])" + re.escape(word) + r"[А-Яа-яЁёA-Za-z]*",
            "", out, flags=re.IGNORECASE)
    # Пробелы и знаки, оставшиеся от выкинутого слова: «купи  за» и « ,»
    out = re.sub(r"[ \t]{2,}", " ", out)
    out = re.sub(r"\s+([,.!?;:])", r"\1", out)
    return out.strip()


def words_are_gone(text: str, words) -> bool:
    """Не осталось ли запрещённого в тексте — подстрокой, как ищет панель.

    Проверка обязательна: предлагать «убрать и создать», не убедившись, что
    убрали, значит обещать исход, которого не будет.
    """
    low = str(text or "").lower()
    return not any(str(w or "").lower() in low for w in words if w)
