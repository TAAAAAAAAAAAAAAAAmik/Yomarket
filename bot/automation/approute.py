...
_CODES = {
    ...
    "FORBIDDEN": "ключу не хватает прав, либо наш IP не в белом списке "
                 "кабинета — посмотри «🪪 Наш IP у поставщика»",
...
_HTTP = {
    401: "ключ не принят — проверь срок ключа, белый список IP и кабинет",
...
def _no_envelope(body: dict, http: int) -> str:
    ...
    out += (f"\nЧто было в ответе: {keys}"
            f"\nЭто не тот конверт, что описан в их SDK. Покажи этот "
            f"отчёт поставщику или посмотри <code>/apr_debug</code>.")
    return out
...
def _wait_note(seconds: float) -> str:
    ...
    if seconds and seconds > 0:
        return f" — подожди {int(seconds) + 1} с"
    return " — лимит считается на минуту, попробуй через минуту"
        ...
        try:
            ...
            if row["complaints"]:
                ...
                row["reading"] = ("отказа не назвали. Считать это согласием "
                                  ...
                                  "не понят — смотри data и statusCode")
            else:
