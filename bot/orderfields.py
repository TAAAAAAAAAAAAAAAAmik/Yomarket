...
def parse_amount(value):
    ...
    **Запятая у этой панели — десятичная, всегда.** Разряды она отбивает
    пробелом. Это выяснено с продавцом на живом магазине: «586,226 ₽» —
    пятьсот восемьдесят шесть рублей, а не пятьсот восемьдесят шесть тысяч.
    Правило «после запятой три цифры — значит разряды» выглядело разумным,
    но именно оно превратило баланс в 586 226 ₽ — в тысячу раз больше, чем
    есть, и опять на экране продавца. То же правило действует в `_num`
    (`automation/panel.py`), и разъезжаться им нельзя.

    Запятая читается как разрядная ровно в одном случае — когда в числе
    есть ещё и точка: «1,234.56» иначе не разобрать.
    """
...
def money(value: float | None) -> str:
    """1234.0 → «1 234», 586.226 → «586,226», None → «».

    Копейки отделяются запятой, разряды — пробелом, как их пишет панель.
    Знаков после запятой до трёх: панель показывает баланс как «586,226 ₽»,
    и урезать его до «586,23 ₽» значит расходиться с ней там, где вся суть
    экрана — совпадать.
    """
    if value is None:
        return ""
