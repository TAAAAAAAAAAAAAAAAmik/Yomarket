...
def _plugins_menu_text(settings: dict, shop_name: str = "") -> str:
    ...
    # Ради чего это всё. Каждая строка — про устройство бота, а не про
    # будущие доходы: «продажи вырастут» проверить нельзя, а «заказ в четыре
    # утра не ждёт до утра» продавец проверит в первую же ночь. Обещание,
    # которого нельзя сдержать, в этом проекте дороже любой недосказанности.
    lines += [
        "",
        "<b>Что делают плагины</b>",
        "🌙 <b>Работают, пока вы спите.</b> Заказ в четыре утра не ждёт до "
        "утра: код уходит покупателю сам, в любой час.",
        "⏱ <b>Снимают ручную работу.</b> Выдать вручную — это сходить к "
        "поставщику, купить, скопировать код, найти чат, вставить. Бот "
        "проходит этот путь за секунды и не путает номиналы.",
        "⭐ <b>Скорость — это отзывы.</b> За цифровой товар платят как раз "
        "за то, что он приходит сразу; ожидание — первая причина споров и "
        "плохих оценок.",
        f"🎁 <b>{len(cards())} семейств карт на одном движке</b> — Apple, "
        "Xbox, Steam, PlayStation, Roblox, Amazon и другие. Новая "
        "подключается одним нажатием, без настройки под каждую.",
        "📜 <b>Всё записано.</b> Что куплено, почём, какой код ушёл. Не "
        "выдалось — бот назовёт причину, а не промолчит.",
    ]
    return "\n".join(lines)
