...
def parse_creds(text: str) -> dict:
    ...
    Ключ у AppRoute один и узнаётся по виду (`sk_live_…`), поэтому вставку
    без подписи понимаем как ключ: требовать `api_key:` перед единственным
    значением — требовать аккуратности там, где угадать нечего.
    """
    out: dict = {}
...
async def apr_login(message: Message) -> None:
    ...
    if not got:
        await message.answer(
            ...
            "<code>/apr_login sk_live_ваш_ключ</code>\n\n"
...
async def apr_set_key(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(
        ...
        "Кабинет → <b>Dashboard</b> → раздел с ключами. Выглядит как "
        "<code>sk_live_…</code>.\n\n"
        "Пришлите его одним сообщением — удалю сразу после разбора.",
