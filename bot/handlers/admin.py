...
def _menu_kb(uid: int):
    ...
    b.button(text="₿ Оплата Bybit", callback_data="admin:bybit")
...
# засчитывает оплату. Здесь задаётся всё, что для этого нужно.
#
# Секрет ключа на экран не выводится НИКОГДА — ни целиком, ни началом. Ключ
# на чтение показывает все поступления владельца, то есть его выручку;
# печатать его в чат, который потом пересылают в поддержку, нельзя.

@router.callback_query(F.data == "admin:bybit")
async def bybit_menu(callback: CallbackQuery, state: FSMContext) -> None:
    if not is_admin(callback.from_user.id):
        await callback.answer("Нет доступа", show_alert=True)
        return
    await state.clear()
    from storage import (bybit_trouble, get_bybit, get_usdt_prices,
                         unresolved_payments, usdt)
    b_ = get_bybit()
    prices = get_usdt_prices()
    unresolved = unresolved_payments()

    body = [
        f"🆔 Твой UID: <b>{ui.esc(b_.get('uid') or '—')}</b>",
        # Только «задан» или «нет»: длина и первые знаки ключа — это тоже
        # сведения о ключе, а пользы от них на экране никакой.
        "🔑 Ключ API: <b>"
        + ("задан" if b_.get("key") and b_.get("secret") else "нет") + "</b>",
        "",
        "<b>Цены в USDT</b>",
    ]
    ...
    for days, label in PRICE_TIERS:
        price = prices.get(days)
        body.append(f"{'✅' if price else '○'} {label} — "
                    + (f"<b>{usdt(price)} USDT</b>" if price
                       else "<i>не задана, оплатить нельзя</i>"))
        kb.button(text=("✏️ " if price else "➕ ") + label,
                  callback_data=f"admin:byb:tier:{days}")

    trouble = bybit_trouble()
    body += ["", ("🟢 <b>Приём работает</b>" if not trouble
                  else f"🔴 <b>Не работает</b> — {trouble}")]
    if unresolved:
        body.append(f"⚠️ Неразобранных поступлений: <b>{len(unresolved)}</b>")

    kb.button(text="🆔 Мой UID", callback_data="admin:byb:uid")
    kb.button(text="🔑 Ключ API", callback_data="admin:byb:key")
    kb.button(text="🧪 Проверить ключ", callback_data="admin:byb:check")
    kb.button(text=("🔴 Выключить приём" if b_.get("enabled")
                    else "🟢 Включить приём"), callback_data="admin:byb:toggle")
    if unresolved:
        kb.button(text="⚠️ Неразобранные", callback_data="admin:byb:unres")
    kb.button(text="⬅️ Назад", callback_data="admin:menu")
    await callback.message.edit_text(
        ui.screen("₿ <b>Оплата через Bybit</b>", body,
                  footer="<i>Продавец переводит USDT на твой UID внутри "
                         "Bybit — мгновенно и без комиссии. Бот видит "
                         "поступление и засчитывает оплату сам.</i>"),
        reply_markup=ui.lay(kb).as_markup())
    await callback.answer()


@router.callback_query(F.data == "admin:byb:uid")
async def bybit_uid_start(callback: CallbackQuery, state: FSMContext) -> None:
    if not is_admin(callback.from_user.id):
        await callback.answer("Нет доступа", show_alert=True)
        return
    await state.set_state(AdminState.bybit_uid)
    b = InlineKeyboardBuilder()
    b.button(text="⬅️ Назад", callback_data="admin:bybit")
    await callback.message.edit_text(ui.screen(
        "🆔 <b>Твой UID на Bybit</b>",
        ["Его увидят продавцы — на этот номер они переводят USDT.", "",
         "Взять: приложение Bybit → «Профиль», UID под именем. "
         "Только цифры."]),
        reply_markup=ui.lay(b).as_markup())
    await callback.answer()


@router.message(AdminState.bybit_uid)
async def bybit_uid_save(message: Message, state: FSMContext) -> None:
    if not is_admin(message.from_user.id):
        return
    from automation.bybit import parse_uid
    from storage import set_bybit
    ...
    set_bybit(uid=uid)
    await state.clear()
    await message.answer(f"✅ UID сохранён: <code>{uid}</code>")
    await _show_menu(message, message.from_user.id, edit=False)


@router.callback_query(F.data == "admin:byb:key")
async def bybit_key_start(callback: CallbackQuery, state: FSMContext) -> None:
    if not is_admin(callback.from_user.id):
        await callback.answer("Нет доступа", show_alert=True)
        return
    await state.set_state(AdminState.bybit_key)
    b = InlineKeyboardBuilder()
    b.button(text="⬅️ Назад", callback_data="admin:bybit")
    await callback.message.edit_text(ui.screen(
        "🔑 <b>Ключ API Bybit</b>",
        ["Пришли одной строкой: <code>ключ секрет</code> — через пробел.",
         "",
         "<b>Ключ нужен только на чтение.</b> Право выводить средства не "
         "включай: боту достаточно видеть поступления, а ключ с выводом — "
         "это доступ к твоим деньгам.",
         "",
         "Если у ключа задан белый список IP, впиши туда адрес сервера, "
         "иначе Bybit не пустит."],
        footer="<i>Сообщение с ключом удалю сразу после сохранения.</i>"),
        reply_markup=ui.lay(b).as_markup())
    await callback.answer()


@router.message(AdminState.bybit_key)
async def bybit_key_save(message: Message, state: FSMContext) -> None:
    if not is_admin(message.from_user.id):
        return
    from storage import set_bybit
    parts = (message.text or "").split()
    # Сообщение с ключом убирается из переписки СРАЗУ и независимо от того,
    # приняли мы его или нет: неверный ключ — такой же секрет, как верный.
    try:
        await message.delete()
    except Exception:                              # noqa: BLE001
        pass
    if len(parts) != 2:
        await message.answer("Нужны два слова: ключ и секрет через пробел. "
                             "Пришли ещё раз.")
        return
    set_bybit(key=parts[0], secret=parts[1])
    await state.clear()
    await message.answer("✅ Ключ сохранён, сообщение с ним удалил.\n"
                         "Проверь его кнопкой «🧪 Проверить ключ».")
    await _show_menu(message, message.from_user.id, edit=False)


@router.callback_query(F.data == "admin:byb:check")
async def bybit_check(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    from automation.bybit import check_access
    from storage import get_bybit
    creds = get_bybit()
    if not (creds.get("key") and creds.get("secret")):
        await callback.answer("Ключ не задан", show_alert=True)
        return
    await callback.answer("Спрашиваю Bybit…")
    ok, why = await asyncio.to_thread(check_access, creds["key"],
                                      creds["secret"])
    b = InlineKeyboardBuilder()
    b.button(text="⬅️ Назад", callback_data="admin:bybit")
    await callback.message.edit_text(
        ui.screen(("✅ <b>Ключ работает</b>" if ok
                   else "❌ <b>Ключ не принят</b>"), [ui.esc(why)]),
        reply_markup=ui.lay(b).as_markup())


@router.callback_query(F.data == "admin:byb:toggle")
async def bybit_toggle(callback: CallbackQuery, state: FSMContext) -> None:
    if not is_admin(callback.from_user.id):
        await callback.answer("Нет доступа", show_alert=True)
        return
    from storage import bybit_trouble, get_bybit, set_bybit
    on = not get_bybit().get("enabled")
    set_bybit(enabled=on)
    # Включённый приём при незаданном ключе — кнопка «оплатить», ведущая в
    # пустоту. Говорим об этом сразу, а не оставляем выяснять по жалобам.
    trouble = bybit_trouble()
    await callback.answer(
        ("Приём включён." if on else "Приём выключен.")
        + (f"\n\nНо {trouble} — продавцам оплата пока не предлагается."
           if on and trouble else ""), show_alert=True)
    await bybit_menu(callback, state)


@router.callback_query(F.data.startswith("admin:byb:tier:"))
async def bybit_tier_start(callback: CallbackQuery, state: FSMContext) -> None:
    if not is_admin(callback.from_user.id):
        await callback.answer("Нет доступа", show_alert=True)
        return
    try:
    ...
    if not label:
        await callback.answer()
        return
    from storage import get_prices, get_usdt_prices, usdt
    await state.set_state(AdminState.bybit_price)
    await state.update_data(bybit_days=days)
    cur = get_usdt_prices().get(days)
    rub = get_prices().get(days)
    b = InlineKeyboardBuilder()
    b.button(text="⬅️ Назад", callback_data="admin:bybit")
    await callback.message.edit_text(ui.screen(
        f"₿ <b>{label} — цена в USDT</b>",
        [f"Сейчас: <b>{usdt(cur)} USDT</b>" if cur else "Сейчас: не задана",
         (f"В рублях этот тариф стоит <b>{rub} ₽</b>." if rub else
          "Рублёвая цена у этого тарифа не задана."),
         "",
         "Введи цену в USDT числом, можно с точкой."],
        footer="<i>Курс не считается: цену назначаешь ты. Иначе при скачке "
               "курса продавец заплатил бы не то, что видел на экране.</i>"),
        reply_markup=ui.lay(b).as_markup())
    await callback.answer()


@router.message(AdminState.bybit_price)
async def bybit_tier_save(message: Message, state: FSMContext) -> None:
    if not is_admin(message.from_user.id):
        return
    from storage import set_usdt_price
    data = await state.get_data()
    days = int(data.get("bybit_days") or 0)
    try:
        price = float((message.text or "").replace(",", ".").strip())
    except ValueError:
        await message.answer("Нужно число, например 3 или 4.5.")
        return
    if price < 0:
        await message.answer("Отрицательная цена — это не цена.")
        return
    set_usdt_price(days, price)
    await state.clear()
    label = dict(PRICE_TIERS).get(days, str(days))
    from storage import usdt
    await message.answer(f"✅ {label}: " + (f"{usdt(price)} USDT"
                                           if price else "цена убрана"))
    await _show_menu(message, message.from_user.id, edit=False)


@router.callback_query(F.data == "admin:byb:unres")
async def bybit_unresolved(callback: CallbackQuery, state: FSMContext) -> None:
    """Поступления, которые бот не смог приписать продавцу.

    Деньги ПРИШЛИ, и молчать о них нельзя. Экран нужен для того, чтобы
    владелец мог выдать дни руками, а не искать концы в кабинете Bybit.
    """
    if not is_admin(callback.from_user.id):
        await callback.answer("Нет доступа", show_alert=True)
        return
    await state.clear()
    from storage import unresolved_payments, usdt
    items = sorted(unresolved_payments().items(),
                   key=lambda kv: kv[1].get("at", 0), reverse=True)[:15]
    body = []
    for _tx, r in items:
        body.append(f"• <b>{usdt(r.get('amount'))} "
                    f"{ui.esc(str(r.get('coin')))}</b> "
                    f"от UID <code>{ui.esc(str(r.get('uid') or '—'))}</code> — "
                    f"{ui.esc(str(r.get('reason')))}")
    b = InlineKeyboardBuilder()
    b.button(text="🎫 Выдать подписку", callback_data="admin:sub")
    b.button(text="⬅️ Назад", callback_data="admin:bybit")
    await callback.message.edit_text(
        ui.screen("⚠️ <b>Неразобранные поступления</b>",
                  body or ["Пусто — все поступления разобраны."],
                  footer="<i>Названный продавцом UID разберёт их сам. "
                         "Если не дождаться — выдай дни вручную.</i>"),
        reply_markup=ui.lay(b).as_markup())
    await callback.answer()
