...
BOT_VERSION = "2026-09-07-copy-asks-panel-by-name"
...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    import logs
    from storage import note_visit

    # Запись «зашёл впервые» — одна на человека, и решает это отдельная
    # отметка. Прежде спрашивалась отметка ПРОБЫ: не бравший её попадал в
    # журнал на каждый свой заход, и «впервые» стояло на пятом.
    #
    # Второй строки, «вернулся, магазин не подключён», не стало вовсе.
    # Действия за ней никакого — сделать по такой записи нечего, — а
    # приходила она на каждое `/start`, то есть чаще всего тогда, когда
    # человек растерян и жмёт его подряд. В этом шуме тонули записи, ради
    # которых журнал и заведён: просьбы о счёте и оплаты.
    if note_visit(message.from_user.id):
        await logs.log_event(message.bot, "users", ["Зашёл в бота впервые."],
