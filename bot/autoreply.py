...
# Блокировка магазина — беда не экрана, а всего бота: пока она стоит, не
# читаются ни объявления, ни заказы, ни чаты, и ни одна авто-функция не
# работает. Текст живёт ОДНОЙ строкой на весь проект: его показывают экран
# объявлений, подключение магазина, разбор отказов и фоновый проход, и
# разойдясь, они советовали бы разное про одну и ту же беду.
#
# Главное в нём — что токен ни при чём. Все соседние отказы ведут к «создай
# токен заново», и продавец, не получив другого ответа, пойдёт делать именно
# это: потратит время и ничего не починит.
SHOP_BLOCKED = (
    "маркетплейс заблокировал магазин. Это решение Юмаркета, а не бота: "
    "пока блокировка стоит, не читаются ни объявления, ни заказы, ни чаты, "
    "и авто-функции не работают. Токен ни при чём — новый не поможет. "
    "Снять блокировку и назвать причину может только поддержка Юмаркета")


def shop_blocked(err) -> str:
    """Про заблокированный ли это магазин → текст или пусто.

    Отдельной функцией, а не сравнением строк по месту: читателей четверо, и
    четыре одинаковых условия однажды разойдутся — например, когда
    маркетплейс начнёт писать код иначе.
    """
    return SHOP_BLOCKED if "shop_blocked" in str(err or "").lower() else ""


_SEND_ERRORS: tuple[tuple[str, str, bool], ...] = (
    # Третьим значением False: «ответьте вручную» здесь — тот же совет
    # биться в стену, что и при закрытом чате. Заблокированный магазин не
    # чинится действиями продавца в кабинете.
    ("shop_blocked", SHOP_BLOCKED, False),
