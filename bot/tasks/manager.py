...
# «куплен, ждём код» — заказ принят поставщиком (`IN_PROGRESS`), но код к
# концу прохода ещё не пришёл; ожидание ограничено сверху, потому что на
# этом же потоке стоит опрос заказов продавца.
...
class TaskManager:
    ...
    # ------------------------------------------------------------------
    #
    # Политика конфиденциальности обещает: данные хранятся три календарных
    # дня после окончания подписки и удаляются, если она не продлена.
    # Обещание, которого код не выполняет, — не недоделка, а ложь в
    # документе, на который сошлются в споре.
    #
    # Проход один на весь бот, а не по продавцу: он ходит по списку подписок
    # целиком, и заводить его копию на каждого — значит стирать одно и то же
    # столько раз, сколько запущено циклов.
    ...
    async def purge_expired(self, now: float | None = None) -> list[int]:
        ...
        Отдаёт список продавцов, у которых что-то удалено, — чтобы это можно
        было проверить тестом, а не поверить на слово.
        """
        from storage import (expired_before, purge_user,
                             require_subscription_enabled)

        # Пока подписка не требуется, истёкшая ничего не значит: продавец
        # спокойно работает и без неё. Стереть его данные по такой отметке
        # значит снести живой магазин человеку, который ничего не нарушал.
        if not require_subscription_enabled():
            return []

        now = time.time() if now is None else now
        gone: list[int] = []
        for uid in expired_before(now - self.PURGE_AFTER):
            # Сначала остановить проходы этого продавца: они держат его
            # настройки в памяти и сохраняют их в конце прохода — удаление
            # на ходу было бы возвращено обратно секундой позже.
            self.stop_for_user(uid)
            report = purge_user(uid)
            if "отказ" in report or not report:
                continue
            gone.append(uid)
            logger.info("данные продавца %s удалены по истечении подписки: %s",
                        uid, report)
            # Сказать вслух. Молча стёртые данные снаружи неотличимы от
            # сломавшегося бота, а человек вправе узнать, что произошло.
            await self._notify(uid, _card(
                "🗑 <b>ДАННЫЕ УДАЛЕНЫ</b>",
                ["Подписка кончилась больше трёх дней назад, и данные "
                 "удалены — так написано в политике конфиденциальности.",
                 "",
                 "Чтобы начать заново, жми /start и подключи магазин "
                 "снова."],
                "🔑 Отзови выданный боту токен в панели Юмаркета"))
        return gone
    ...
    def _pace_chats(self, user_id: int, settings: dict) -> None:
        ...
        нащупывается: идём с пола, на «сбавь темп» отступаем вдвое, а после
        ...
        try:
            ...
            for order in orders:
                ...
                if prev_status is None:
                    ...
                    # следующую правку AppRoute придётся класть дважды, и
                    # первое же расхождение было бы молчаливым.
                    robux_tried.add(oid)
            ...
            # Сначала то, о чём продавец попросил явно, потом то, что
            # оборвалось само. Один проход на все карты, Robux в их числе.
    ...
    async def _maybe_accept_order(self, user_id: int, api: YooMarketAPI,
        ...
        прямо предупреждает «не выдавай товар», а бот брал его в работу.
        ...
        for oid, status in list(known.items()):
                ...
                await self._notify(
                    ...
                    _card("⚠️ <b>АВТОВОЗВРАТ НЕ ПРОШЁЛ</b>",
                          [f"📦 {_esc(title)} <code>#{_esc(str(oid))}</code>",
                           ...
                           "",
                           "Заказ остался как был — верни вручную, если "
                           "нужно."]))
            ...
            if confirmed and confirmed in _BACK_STATUSES:
                ...
                tail = (f"Маркетплейс всё ещё показывает «{_status_ru(confirmed)}». "
                        f"Проверь заказ вручную.")
            else:
                head = "↩️ <b>АВТОВОЗВРАТ ОТПРАВЛЕН</b>"
                tail = "Статус перечитать не удалось — проверь заказ сам."
            await self._notify(
                user_id,
                _card(head,
                      [f"📦 {_esc(title)} <code>#{_esc(str(oid))}</code>",
                    ...
                    await self._notify(
                        ...
                        _card("⏸ <b>НЕ ПОДТВЕРЖДАЮ — ТОВАР НЕ ВЫДАН</b>",
                              [f"📦 {_esc(det.get('title', f'Заказ #{oid}'))} "
                               ...
                               "товар уйдёт покупателю, — или выдай "
    ...
    # приходит сразу, а ждать его иногда приходится минутами. Ни один шаг не
    # меньше 12 секунд: у `GET /orders?unhide=true` лимит 5 запросов в
            ...
            if not codes:
                ...
                await self._notify(user_id, _card(
                    ...
                    [f"Заказ #{_esc(order_id)}, {_esc(nominal)}.",
                     ...
                     "",
                     "Деньги могли списаться. Посмотри кабинет по этой "
                     "ссылке и выдай код вручную, если он там есть."]))
                return
        ...
        if not sent:
            ...
            await self._notify(user_id, _card(
                ...
                [f"Заказ #{_esc(order_id)}, {_esc(nominal)}.",
                 ...
                 "",
                 "Передай его покупателю сам — второй раз бот покупать "
                 "не станет."]))
            return
            ...
            for oid in waiting:
                ...
                if not node:
                    ...
                    await self._gift_stop(
                        ...
                        + (f" ({err})" if err else "")
                        + ". Проверь номер; если это был сбой связи — "
                        "поставь заказ в очередь снова", record=False)
                    continue
                d = _describe(node)
    ...
    async def _gift_stop(self, user_id: int, settings: dict, gift,
        ...
        await self._notify(user_id, _card(
            ...
            [f"Заказ #{_esc(order_id)}"
             ...
             "",
             "Покупатель ждёт оплаченный заказ — выдай вручную."]))
            ...
            await self._notify(
                ...
                _card("⭐ <b>ЗВЁЗДЫ НЕ ВЫДАНЫ</b>",
                      [f"Заказ #{_esc(order_id)}, @{_esc(username)}, {qty}⭐",
                       ...
                       "Пополни кошелёк и выдай вручную: "
        ...
        if ok:
            ...
            if spend.get("sent_onchain"):
                ...
                await self._notify(
                    ...
                    f"{result}\n\n"
                    "Повтор не делаю: он купил бы звёзды второй раз за твои "
                    "деньги. Проверь на fragment.com, начислены ли звёзды, "
                    "и выдай вручную только если их нет: "
                    "Плагины → AutoStars → 🚀 Ручная выдача",
                )
                return True
            if timed_out:
                ...
                await self._notify(
                    ...
                    "второй раз. Проверь на fragment.com, начислены ли "
                    "звёзды, и выдай вручную только если их нет: "
                    "Плагины → AutoStars → 🚀 Ручная выдача",
                )
                return True
            ...
            if (stars_notify_on(settings, "failed")
                ...
                tail = ("\n\n<b>Попытки исчерпаны</b> — заказ ждёт тебя."
                ...
                await self._notify(
                    ...
                    + "Выдай вручную: Плагины → AutoStars → 🚀 Ручная выдача",
                )
        return True
    ...
    async def _stars_pending_sweep(self, user_id: int, api: YooMarketAPI,
        ...
        return _card("⭐ <b>ЗВЁЗДЫ: ЖДУТ USERNAME</b>", lines,
                     "Покупатели не прислали @username. Напиши им или "
                     "выдай вручную: Плагины → AutoStars")
    ...
    async def _stars_session_watch(self, user_id: int, settings: dict,
        ...
        return _card("⭐ <b>СЕССИЯ FRAGMENT ИСТЕКЛА</b>",
                     [f"Проверил: {_esc(why)}.",
                      ...
                      "",
                      "Пересними куки: Плагины → AutoStars → ⚙️ Настройки "
                      "→ 🔑 Данные Fragment."])
    ...
    async def _stars_balance_watch(self, user_id: int, settings: dict,
        ...
        return _card("⭐ <b>ЗАКАНЧИВАЕТСЯ TON</b>",
                     [f"На кошельке <b>{bal['ton']:.3f} TON</b>",
                      ...
                      "",
                      "Пополни кошелёк, иначе выдача остановится на "
                      "оплаченном заказе."])
    ...
    async def _auto_answer(self, user_id: int, api: YooMarketAPI, settings: dict,
        ...
        if ok:
            ...
            if now - float(conf.get("last_fail_notice", 0) or 0) > 3600:
                ...
                tail = ("Покупателю ничего не отправлено — ответь вручную."
                        if fixable else
    ...
    async def _panel_bump(self, user_id: int, api: YooMarketAPI | None = None,
        ...
        creds = get_panel_creds(user_id)
        if not creds or not creds.get("cookies"):
            return 0, "нужен вход в панель — открой «Панель продавца»"

        settings = get_settings(user_id)
        params = promo_params(settings)
        if not params:
            return 0, ("не выбран тариф «Премиум» — открой "
    ...
    async def _auto_restore(self, user_id: int, api: YooMarketAPI,
        ...
        if new_unknown:
            ...
            body.append("<i>Напиши нам этот статус, если такие товары "
    ...
    async def _check_panel_session(self, user_id: int, settings: dict, now: float) -> None:
        ...
        if not ok and was_ok:
            ...
            await self._notify(
                ...
                "Войди заново — код придёт на почту.",
