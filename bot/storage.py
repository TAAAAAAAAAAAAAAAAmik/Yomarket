...
_WHAT_I_DO = (
    "<b>💬 Покупатель</b>\n"
    "• Отвечу на сообщение сразу — по твоим правилам, пока он не убежал "
    "к соседу\n"
    "• Напишу при новом заказе и при возврате\n"
    "• Замечу жалобу или спор первым — до арбитража, а не после\n\n"

    "<b>📦 Заказы</b>\n"
    "• Возьму в работу за секунды — хоть в четыре утра\n"
    "• Подтвержу через заданное время\n"
    "• Напомню о зависших\n\n"

    "<b>🚀 Объявления — держу наверху</b>\n"
    "• Слежу за позицией на витрине и поднимаю, как только просядешь: "
    "объявление стоит в топе, а не «поднято утром и забыто»\n"
    "• Подниму «Премиумом» по расписанию или каждые N часов\n"
    "• Подниму весь пак разом\n"
    "• Верну распроданное сразу после продажи\n"
    "• Восстановлю снятые\n"
    "• Выложу новый товар — с тебя название и цена, остальное моё\n\n"

    "<b>🧩 Плагины — выдача без тебя</b>\n"
    "• Гифт-карты, {карт} штук: {карты}\n"
    "• Telegram Stars — куплю и отправлю сам\n"
    "• Код уходит покупателю в чат за секунды, в любое время суток\n\n"

    "<b>📊 Деньги</b>\n"
    "• Выручка и чистыми — каждый вечер, без таблиц\n"
    "• Скажу, когда баланс дойдёт до порога\n"
    "• Покажу новые отзывы\n\n"

    "<b>⚡️ И всё это — из Telegram</b>\n"
    "• «Автопилот» включает рабочий набор одной кнопкой\n"
    "• Несколько магазинов на одном аккаунте\n"
    "• Ставить нечего: бот и есть панель управления"
)
...
CUSTOM_TEXTS = {
    "welcome": {
        ...
        "default": (
            ...
            "кто-то оформляет заказ в твоём магазине и ждёт ответа. Не "
            "дождался за час — ушёл к тому, кто ответил быстрее. Вот эти "
            "часы я и закрываю.\n\n"
            + _WHAT_I_DO + "\n\n"
            "<b>Рутина на мне. Продажи твои.</b>\n\n"
            "━━━━━━━━━━━━━━\n"
            "🆓 <b>Бесплатно и сразу:</b> подключить магазин, получать "
            "уведомления о заказах и посмотреть, как всё устроено изнутри.\n\n"
...
PRICE_TIERS: tuple[tuple[int, str], ...] = (
    ...
    (14, "2 недели"),
    (30, "1 месяц"),
    # Трёхмесячного тарифа нет намеренно (решение владельца 31.08): между
    # месяцем и полугодием он ничего не добавлял, а список выбора делал
    # длиннее. Цена, оставшаяся в хранилище от снятого срока, никому не
    # показывается — `get_prices` перебирает этот кортеж, а не хранилище.
    (180, "6 месяцев"),
    (365, "12 месяцев"),
)
...
def start_trial(user_id: int, days: int | None = None,
    ...
    Срок можно назвать явно: проб две — короткая без условий и длинная за
    подписку, — а отметка «уже брал» у них ОДНА. Взял короткую, значит
    длинная уже не положена: иначе десять дней вместо семи получал бы любой,
